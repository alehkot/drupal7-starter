...
//D8:namespace Drupal\inspect;

// Drupal 7's autoloader cannot include this file; presumably because the class names are the same (no namespace support).
...
class Inspect extends \SimpleComplex\inspect\Inspect {
   ...
   *
   * @param boolean $check
   *   Truthy: attempts to read database max query length; fall-back Inspect::OUTPUT_DEFAULT.
   *   Default: FALSE (~ gets from variable setting (or database max query length) and subtracts some for safe margin).
   *
   * @return integer
   */
  public static function outputMax($check = FALSE) {
    $setting = static::configGet('inspect_output_max', 0);

    if ($setting && !$check) {
      // Subtract 5% for database abstraction query manipulation (seen 1.5% difference).
    ...
    if (!$setting) {
      ...
      static::configSet('inspect_output_max', $le > static::OUTPUT_DEFAULT ? static::OUTPUT_DEFAULT : $le);
    }
